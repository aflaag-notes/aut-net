		...
		\begin{enumerate}
			...
			\item \tbf{Route request diven optimization context}:
			      ...
			      \begin{itemize}
				      ...
				      \item each node can decide to partecipate or not: if a node agrees to partecipate, it selects thje next node to forward the RR to, and this process continues until $d$ is reached
		...
		\begin{enumerate}
			...
			\item \tbf{Routing protocol driven optimization context}:
			      ...
			      \begin{itemize}
				      ...
				      \item in each $\Delta t$ the central node selects a routing protocols and calculates the routing tables, which are then sent to slave nodes for internal configuration
...
\begin{frame}
	...
	Categorizing between \tbf{unicast or multicast} approaches is a natural choice, given the inherent \tit{overhead} that multicast routing protocols introduce. \\
	\vspace{0.8cm}
...
\begin{frame}
	\frametitle{Example of a Multicast protocol}
	\framesubtitle{Classification criteria: Context of use}
	...
	\begin{itemize}
		...
		\item routing to multiple destinations is defined as the \tbf{minimum cost path} starting at the source and reacting all destinations interested
		\item the \tit{cost} of a spanning tree is defined as the number of one-hop broadcasts to reach all sinks
	\end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{Example of a multi-metric protocol}
	\framesubtitle{Classification criteria: Context of use}
...
\begin{frame}
	\frametitle{Example of a delay-aware protocol}
	\framesubtitle{Classification criteria: Context of use}
...
\begin{frame}
	\frametitle{Example of a model-based protocol}
	\framesubtitle{Classification criteria: Design characteristics}
	...
	The assumption is that in order to reach a some destination $d$ located in a given grid it is possible to infer the optimal paths to cross from the \tbf{history of inter-grid movements}.
\end{frame}
...
\begin{frame}
	...
	\begin{overprint}
		...
		In particular \tit{collaboration} concerns both RL and non-RL related exchanges, such as the exchange of \tbf{link-state information}.
...
\begin{frame}
	\frametitle{Example of a proactive protocol}
	\framesubtitle{Classification criteria: Design characteristics}
...
\begin{frame}
	\frametitle{Example of a hybrid protocol}
	\framesubtitle{Classification criteria: Design characteristics}

	An example of a protocols that combines RL with other optimization techniques is \tbf{AdaR} (the first multi-metric protocol) \cite{wang}. \\
...
\begin{frame}
	...
	\begin{overprint}
		...
		\begin{itemize}
			\item \tbf{low overhead}: the chosen next hop returns a feedback in an explicit ACK packet, or it includes its feedback when, in turn, it (re)forwards the packet --- Half of the reviewed protocols fall under this category
		\end{itemize}
...
\begin{frame}
	...
	\begin{overprint}
		...
		\begin{itemize}
			\item \tbf{high overhead}: when the state space is a list of whole paths with thier current characteristics
		\end{itemize}
	\end{overprint}
\end{frame}
...
\begin{frame}
	...
	\begin{overprint}
		...
		\begin{itemize}
			\item \tbf{very high overhead}: when the state space depends on combinations of channels subsets or paths TODO WHAT?
		\end{itemize}
	\end{overprint}
\end{frame}
...
\begin{frame}
	...
	It would be clearly important to categorize the protocols based on the \tbf{performance} based on the simulations. However, given the
...
\begin{frame}{Conclusion}
	...
	\begin{itemize}
		\item provide \tbf{higher level} of QoS
...
\begin{frame}{Speed of convergence}
	...
	convergence rates should be investigated to provide \tbf{bounds} of delay for let users know when the network can or cannot provide \tit{acceptable QoS levels}.
\end{frame}

\begin{frame}{Link-state information dissemination}
	In most protocols, \tbf{link-state information} is used to calculate metrics, hence the convergence of the routing algorithms strictly depends on the \tit{freshness} of disseminated link-state information. \\
