...
\begin{frame}{Motivation}
	...
	As a result, \tbf{Machine Learning (ML)} is increasingly used to handle tasks such as
...
\begin{frame}{The routing problem}
	In networks, \tbf{routing} is the problem of sleecting paths to send packets from source(s) to destination(s), while
	...
	However, whenever multiple metrics are required, the routing problem becomes \textsf{NP}-complete. \\
...
\begin{frame}{Reinforcement Learning}
	...
	For the purpose of finding better solutions for \tbf{routing} in networks, RL-based algorithms seem particularly promising because TODO
\end{frame}

\begin{frame}{Q-learning}
	In 1989 \textcite{watkins} proposed the most-widely adopted flavour of Reinforcement Learning, called \tbf{Q-learning}. \\
	...
	\begin{overprint}
		...
		Very importantly, his approximation of the action function is \tit{independent of the policy} followed by the agents, making Q-learning applicable in a wide variety of contexts.

		\onslide<3>

		The action-value is updates through the following formula: $$Q_n(s_n, a_n) = (1 - \alpha) \cdot Q_{n - 1}(s_n, a_n) + \alpha \cdot \sbk{R_n + \gamma \cdot \max_{a \in \mathcal A}{Q_{n - 1}(s_{n + 1}, a)}}$$ where $\alpha$ is the \tbf{learning factor}, and $\gamma$ is the \tbf{discount rate}. \\

		\vspace{0.8cm}

		The initial Q-values $Q_0(s, a)$ are assumed given.
		...
		Most importantly, Watkins showed that Q-laerning \tbf{converges} to the optimum action-values with probability 1, as long as all actions are repeatedly sampled in all states. \\

		\vspace{0.8cm}

		Indeed, this is the reason why Q-learning is the most popular and effective larning technique in the field of delayed reinforcement.
	\end{overprint}
\end{frame}
...
\begin{frame}{The work in exam}
	...
	\begin{overprint}
		...
		\begin{enumerate}
			\item provide a \tbf{comprehensive peresentation} of the main characteristics of RL-based routing protocols
		\end{enumerate}
...
\begin{frame}{Table of contents}
	...
	Subsequently, the most important segment of this review will be presented: the \tbf{classification criteria} that the authors defined in order to categorize the literature. \\
...
\begin{frame}
	...
	RL-based algorithms requires \tit{memory} to store the \tbf{states of the agents}, and the number of states may be very high. \\
	...
	\begin{overprint}
		...
		\begin{itemize}
			\item \tbf{very low overhead}: then when state space is states of a packet TODO WHAT
		\end{itemize}

		\onslide<3>

		\begin{itemize}
			\item \tbf{low overhead}: when the state space is the node IDs --- most of the reviewed papers fall under this category
		\end{itemize}

		\onslide<4>

		\begin{itemize}
			\item \tbf{limited overhead}: when the state space depends on external factors --- e.g. the number of transmission power leves, the maximum number of available channels, etc.
		\end{itemize}

		\onslide<5>

		\begin{itemize}
			\item \tbf{high overhead}: when the state space is a list of whole paths withthier current characteristics
...
\begin{frame}
	...
	Additionally, RL-based algorithm also require \tit{memory} store all the \tbf{possible actions} that agents can perform. \\

	\vspace{0.8cm}

	\pause

	Therefore again, the protocols can be \tit{qualitatively} grouped based on the \tbf{action space}:
		...
		\begin{itemize}
			\item \tbf{high overhead}: when the action space depends on either
			      \begin{itemize}
				      ...
				      \item the number of \tit{grids} in the network --- WHAT ARE THESE
			      \end{itemize}
		\end{itemize}
...
\begin{frame}
	...
	In the optimization field, the \tbf{convergence} to optimal solutions is an \tit{expected} property. \\

	\vspace{0.8cm}

	\pause

	Nevertheless, many existing techniques to solve multicriteria optimization problems are not guarantted to reach the optimal solution. \\

	\vspace{0.8cm}

	\pause

	Regearding RL-based algorithms, from the original work of \textcite{watkins} it is possible to derive proofs of convergence of RL algorithms, however:

	\begin{itemize}
		\item not all RL-based approaches are based on the standard implementation of Q-learning
		\item many papers rely on \tbf{additional assumptions} that guarantee convergence
	\end{itemize}
	...
	\begin{overprint}
		...
		Still, in real world scenarios it is hard to establish the \tbf{satisfiability} of such assumptions.

		\onslide<2>

		Proving convergence rigorously remains an issue for most protocols that are not perfectly Q-learning compliant

		\onslide<3>

		Rather, convergence is usually assessed from \tbf{simulations} or just \tit{stated as reachable eventually} without providing any proof of such claims.
	\end{overprint}
\end{frame}
...
\begin{frame}
	...
	It would be clearly important to categorize the protocols based on the \tbf{performance} based on the simulations. \\
	...
	Therefore, they limited their effor to present the data reported by the origianl authors \tit{as-is}.
\end{frame}
